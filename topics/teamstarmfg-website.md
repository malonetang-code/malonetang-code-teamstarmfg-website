...
## 2026-09-03 1号方案动效与子页标题已发布
- 用户批准将本地1号方案的克制微动效、40+慢速计数、Logo与产品卡反馈、子页Banner每次进入重播，以及中英文Banner标题统一发布至正式官网；批准评审源为 `b5d0930`，正式源码为 `659a106`。
- 发布号 `20260903154917`、页面标记 `20260903-2` 已上线；1,429项清单、50/50正式路由、本地与公网各24组桌面/手机浏览器、减少动态效果、关键资源、RFQ与视频Range均通过。
- 未发现CDN传播异常；发布前备份位于 `/root/teamstar-backups/20260903154917/site-before.tar.gz`，即时回退目录为 `/www/wwwroot/wordpress.previous-20260903154917`，完整证据见 `deploy_packages/releases/20260903154917/DEPLOYMENT.md`。
...
- 新增 `check:schema` 自动检查；104 个 JSON-LD 块在 48 个页面通过解析和业务边界检查。页面视觉没有新增模块，现有中英文内容、标题规范、产品图和 RFQ 路径保持不变。
- 正式源码提交 `13f68b2`、评审镜像提交 `0a10df7` 已推送；GitHub Pages 工作流 `29898778327` 成功，评审镜像 48/48 页面保持 `noindex`。
- 发布号 `20260722150141` 已完成生产备份、1,283 项哈希回读、46 个公网页面两轮 92/92、12 组生产浏览器和 11 个旧图 404。RFQ 接口、数据库、3 个附件及双收件人配置均未变化。

## 2026-07-22 官网 RFQ 新增销售收件人
- 官网询价及附件通知新增 `rd01@teamstarmfg.com`，现与原收件人 `yiyi@teamstarmfg.com` 同时接收每一封 RFQ 邮件。
- 生产配置只追加收件人；发件地址、SMTP 密钥、数据库、附件存储、30 天留存、失败重试和前端表单均未修改。
- DirectMail SMTP 已接受一封不含客户资料的双收件人配置测试邮件；`rd01` 邮箱的最终收件箱落位未由 Codex 独立读取。
- 配置修改前已备份至 `/etc/teamstar-rfq/config.php.before-sales-recipient-20260722144428`；接口保持 204，数据库仍为 5 条已发送、0 条排队或重试、3 个有效附件。

## 2026-07-22 阶段 2K 木工产品实物 09 灰底修正上线
- 木工图库第 9 张 `dsc01134.jpg` 已将白色背景改为与同组产品一致的中性灰底；刀片轮廓、孔位、齿形、光影和产品像素均保留原始实拍内容。
- 内置图片编辑先做了两版“只换背景”的概念测试，但都出现轻微暗角，因此未采用；正式图使用确定性亮度蒙版合成，不生成或改造产品本体。
- 处理脚本已固定该图的灰底规则，产品检查会验证尺寸及角点、中心孔灰度；其余 85 张产品照片哈希保持不变。
- 版本 `20260722-2k`、正式源码提交 `456ea00`、评审镜像提交 `f941a07` 已发布；评审镜像通过中英文 390 / 768 / 1440px 检查。
- 发布号 `20260722143447` 已完成生产备份、1,283 项哈希回读、46 个公网页面两轮 92/92、6 组像素级生产浏览器和 11 个旧图 404；RFQ 数据库仍为 5 条已发送、0 条重试、3 个有效附件。

## 2026-07-22 阶段 2J 木工刀片封面一致性上线
- 木工机械刀片目录封面与图库 Hero 已由白底 `dsc01134.jpg` 切换为同文件夹的灰底 `dsc00676-1.jpg`，五个产品分类封面的背景、留白和主体尺度恢复一致。
- 白底图没有删除或跨分类移动，仍保留为木工机械刀片图库第 9 张；五类目录和 86 张产品实拍总数保持不变。
- 产品总览拼图已同步更新，并新增五类代表图文件名校验，避免后续重建时意外恢复旧封面。
- 版本 `20260722-2j`、正式源码提交 `4879f2b`、评审镜像提交 `14834c4` 已发布；本地与评审镜像均通过中英文 390 / 768 / 1440px 检查。
- 发布号 `20260722140524` 已完成生产备份、1,313 项哈希回读、46 个公网页面两轮 92/92、12 组生产浏览器和 11 个旧图 404；RFQ 数据库仍为 5 条已发送、0 条重试、3 个有效附件。

## 2026-07-22 阶段 2I 局部可用厂区照片补充上线
- 对 23 张厂区与车间原片进行第二轮局部审查，新增办公及生产楼正面完整图，并从 `051A9197.jpg`、`051A9239.jpg`、`DSC01316.JPG` 中裁切出机加工设备列阵、设备加工区域和激光切割设备三张网页图。
- 处理仅包含固定坐标裁切、方向校正、轻度锐化、尺寸压缩和 EXIF 清理；未使用生成式补画，未移动或改造现场内容，也未添加未经确认的产能、精度或工艺参数。
- 公司页现为 6 张厂区实景；制造能力页新增 3 张设备现场图。版本 `20260722-2i` 已通过构建、内容、产品、质量、厂区证据及 RFQ 自动检查。
- 正式源码提交 `b89ff99`、评审镜像提交 `1001456` 已推送；评审镜像与生产站分别通过 12/12 中英文多视口浏览器检查。
- 发布号 `20260722134758` 已完成生产备份、1,295 项哈希回读、46 个公网页面两轮 92/92 和 11 个旧图 404；RFQ 数据库仍为 5 条已发送、0 条排队或重试。
